...
typedef struct t_SHOP_BaseInfo {
  ...
  char sSecurityStatusFlag[9];        //  期权合约状态信息标签  C8  该字段为8位字符串，左起每位表示特定的含义，无定义则填空格。
									  ...
									  第6位：‘1’表示合约不能进行垂直价差组合策略，‘0’表示可以进行所有的组合策略。*/
  char sAutoSplitDate[9];			  //垂直价差组合策略到期解除日期
									  //该日期表示垂直价差组合策略到期解除的日期，YYYYMMDD。
									  //对于E - 2后波动加挂的合约，此字段仍为E - 2日。到期日调整时，此字段信息自动同步调整。
...
typedef struct t_SHOP_MarketData {
  ...
  T_I64 iTotalValueTrade;             //成交金额  Turnover of this security
